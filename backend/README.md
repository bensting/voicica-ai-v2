...
Audio Settings (speed/volume/pitch) verified end-to-end too, across all three providers with real, non-default values in one request each: Fish Audio (default path, speed 1.5x/pitch 85 via the actual browser UI), Azure (speed 0.7x/volume 30/pitch 20), a Google Chirp3 HD voice (confirms the pitch-unsupported retry actually fires and still succeeds), and a Google Standard voice (pitch supported directly, no retry needed).
