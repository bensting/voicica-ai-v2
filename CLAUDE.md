...
**任务队列整个从 Redis/arq 搬到了 Postgres（ADR 0026），起因是用户自己一句"worker 现在是几秒轮询一次 redis，不可以有任务的时候自动触发吗"**——这个问题问得很准。查了 arq 自己的源码才能给出靠谱答案：它的主循环就是一个 `asyncio.sleep()` 定时器，每次醒来查一次 Redis 有序集合（`ZRANGEBYSCORE`）——不是配置没调好，是这套机制为了支持延迟任务/重试退避/定时 cron，本来就得靠"分数（预定时间）"这个概念，而 `BLPOP` 这种"有任务就立刻醒"的阻塞指令根本表达不了"按分数查询"，所以 arq 从设计上就只能轮询。这个回答直接带出了更大的问题——之前为了省 Redis 命令量把 `poll_delay` 调到 5 秒，用户反馈"Redis 已经满了调用了"，说明这个修法治标不治本：5 个独立轮询循环（4 个 provider 队列 + 1 个 cron）就算调慢了，24/7 常驻跑下来还是远超 Upstash 免费额度。

用户直接提出"方案二更合理，反正没上线，现在改是不是代价最好，也适合未来的发展"——这个判断是对的，思路也跟之前拆分测试库那次一致（趁没有真实用户依赖现有行为，改动成本最低）。讨论过程中用户追问了一个很关键的问题："提交到 kie 还要异步查询进度，这块怎么处理"——倒逼把设计梳理清楚：Kie 的"提交"是真正的队列任务（走新机制），但"等 Kie 跑完"（webhook + 每分钟一次的兜底轮询）从来就不是队列驱动的，是纯粹按时钟触发的定时任务，`LISTEN`/`NOTIFY` 这种事件机制根本表达不了"每隔一分钟查一次"这种需求——这两类工作性质不同，只有前者真的能从轮询改成推送。想清楚这层之后用户又追问"这样队列不需要 Redis了，那保留 Redis 的意义是什么"——如实回答：不是"图省事没搬完"，是 SSE 实时推送这个场景（ADR 0018）Postgres 的 `LISTEN` 需要每个监听者单独占一个不能走连接池的直连，而 Redis 的 pub/sub 天生就能廉价地让很多订阅者共享，这是两种技术真实的优劣势差异，不是能不能砍的问题——而且队列砍掉之后 Redis 剩下的 pub/sub 用量本来就小，不会再撞到额度问题。

**实现前先验证了一个真实的技术风险，没有想当然**：Neon 的连接池模式下 `LISTEN`/`NOTIFY` 会不会正常工作，网上没找到权威说法，直接写了个真实的双连接测试脚本——用 pooler 连接 `LISTEN`、另一个连接 `NOTIFY`，结果**pooler 连接完全收不到通知**（`received=[]`），换成不带 `-pooler` 的直连地址就正常收到了。这个坑提前发现，不然大概率会是"代码写完看起来对、真上线才发现推送根本不生效"的隐藏地雷——加了一个新配置 `DATABASE_DIRECT_URL` 专门给队列监听用，跟原来的 `DATABASE_URL`（连接池，其余地方继续用）分开。

**具体实现**：`jobs` 表里 `status='pending'` 的一行本身就是队列条目，`worker/dispatcher.py`（新写的，替掉了 `worker/settings.py`/`tasks.py`/`run_all.py` 三个文件）用 `SELECT ... FOR UPDATE SKIP LOCKED` 原子认领，被同一个进程里的一根常驻直连 `LISTEN` 连接实时唤醒，另外留了一个 20 秒兜底轮询防止漏掉通知（跟 Kie 自己的 webhook+轮询兜底是同一个思路，这次用在队列本身上）。查 arq 源码还发现一个意外的好消息：它的 `max_jobs` 并发上限内部就是用普通的 `asyncio.Semaphore` 实现的——原来"每个 provider 一个独立队列"这个设计，真正需要的只是"并发上限隔离"，用进程内信号量就能做到，不需要靠拆队列这么重的代价。于是 Fish Audio 4/Azure 20/Google 20/Kie 提交 10 这几个并发上限数字原样保留，只是从"5 个进程各自的队列"变成"1 个进程里的 4 个信号量"。重试机制（arq 原来内置的）也得自己实现：新迁移加了 `jobs.tries`/`jobs.run_after` 两列，一次瞬时错误发生时把状态改回 `pending`、设一个未来的 `run_after` 时间，等到点了被兜底轮询或者别的通知捡回来重跑，退避公式（5 秒 × 第几次重试）跟原来 arq 版本完全一致。定时清理任务（卡住任务清理、Kie 轮询兜底）业务逻辑一行没改，只是从"arq 的 cron 功能"变成"同一个进程里的 `asyncio.sleep()` 循环"——这两个本来就不是队列驱动的活，换底层机制对它们没有任何影响。

**真实验证，不是只看代码逻辑通顺就算数**：真提交一个 TTS 任务，日志里明确打出"Woken by NOTIFY"（不是兜底轮询），直接查数据库 `created_at`→`updated_at` 的真实间隔（不是客户端轮询自己的开销，那个测出来能虚高到 6-7 秒，具体查库才是准的）——热启动之后稳定在 1.3-1.7 秒，比原来 arq 版本默认的轮询间隔更快，而且是真的事件触发不是凑巧轮询命中。真提交一个 Kie 任务，验证了两段设计都在真实工作：提交是 `NOTIFY` 触发的，完成检测是同一个进程里那个不再依赖 Redis 的定时轮询循环真的查到 Kie 结果、真的下载落地。**重试机制这段是意外撞上真实故障验证出来的**：为了测语音克隆的 base64 音频改造，故意传了一段假音频进去，Fish Audio 真的返回了 500（音频格式确实解析不了，不是伪造的错误）——这个 500 触发了真实的重试流程，连续重试两次、每次退避时间都对，第三次达到重试上限后正确写成最终的 `failed` 状态，错误信息也是 Fish Audio 真实返回的——这是这个新机制里最没底的一块（重试/退避逻辑），结果是真被一次意外的真实失败完整验证了一遍，不是靠模拟。

文档同步更新了一圈：新增 ADR 0026，ADR 0014 做了标注（不是删除，指向新 ADR 说明队列机制被替换但其余设计不变），architecture.md 的目录结构和 §3f 的图/文字重新画过，data-model.md 加了 `tries`/`run_after` 两列，backend/README.md 的运行说明、Deploy 表、"已验证"记录都同步了，PyCharm 那几个按 provider 拆开的 Worker 运行配置合并成一个"Worker - Dispatcher"。**还没做的**：Render 生产环境的环境变量和启动命令还没跟着改（`DATABASE_DIRECT_URL` 没配、Background Worker 的 Start 命令还是旧的 `run_all`），这次全程只针对 ADR 0025 那个测试库验证，真要上线还得再走一遍部署这一步。

**`.env.production` 补全 + R2 拆成测试/生产两个桶（ADR 0025 后续），然后引出了一个更大的改动：资产不再经过后端，改成直连 R2 公开域名（ADR 0027）**。用户先指出 `.env.production` "不全，`.env` 变量很多"——之前故意只列了"生产和本地不一样"的变量、其余写"去看本地 `.env`"，这个理由（"避免两处维护同一个密钥"）站不住：文件本来就已经存着生产库密码和 Stripe live key，而且它的目的就是"生产真实值一处能找齐"。用户的划分很清楚："`.env` 只用于本地测试，`.env.production` 用于生产环境（主要是记录）"，于是补成完整的 22 个变量，每一段标明该设在 Render 哪个服务（Web Service / Background Worker）。顺带查代码发现本地和生产共用了几样真实资源，ADR 0025 只拆了 Postgres：Upstash Redis（SSE 频道 `job-events:{user_id}`，Firebase uid 本地生产同一套，本地任务事件会被生产 SSE 收到）和 R2 桶。用户决定 R2 拆开，新建了 `voicica-v2-dev` 桶（开了自己的 `pub-xxx.r2.dev`），原来的 R2 令牌是账号级的、新桶直接能用；真实验证过上传/下载/公开访问/删除一整圈。Redis 先不拆——等"要不要彻底去掉 Redis"这个决定。这里踩了一个小坑：Python 的 `urllib` 默认 User-Agent 被 Cloudflare 拒了（403），curl 同一个 URL 是 200，不是桶的问题。

**用户接着问"生产的 R2 公开地址要不要绑个 voicica.ai 的二级域名"，我一开始回答"不需要，用户碰不到这个地址"——这个回答对当时的代码是对的（`r2.dev` 只用于 Kie 抓参考图），但用户紧接着追问"我们现在前端都是通过后端去拿视频和图片吗？用户一多怎么办"，把问题翻了个面**：查代码确认，`GET /jobs/{id}/asset` 把整个文件读进后端内存再原样返回，前端用带鉴权头的 `fetch` 拿到整个 blob 才显示——视频必须整个下完才能播、不能拖进度条、每个缩略图都是完整原图、公开作品被 100 个人看就从 R2 读 100 次。用户又否掉了"改成流式"这个中间方案（"流式也还是用户一多也有问题"），提出更彻底的：**统一做成开放（不可猜的 URL），3 个月以上的不保存，前端只展示 3 个月内的，"个人项目，用户体验第一"**。这个判断是对的，也符合 CLAUDE.md 开头写的原则。我如实说了代价：`visibility` 的含义从"别人打不开"变成"只是不出现在公开画廊里"，链接一旦分享就撤不回来，隐私页措辞必须跟着改；同时确认文件路径里带的是随机 UUIDv4（122 位）、桶不开放列目录，风险实际很低。

**保留期"写死还是放数据库"，最后选了后者，但关键设计点是"不能只放一个数字"**：过期要在三处生效——R2 里真删文件、数据库、前端展示，而 R2 生命周期规则读不到我们的数据库，如果后台把 90 改成 180、R2 规则还是 90 天，就会出现"前端还显示、文件已经没了"这种最糟的方向。所以做法是：`app_settings.asset_retention_days`（默认 90）→ **创建每个文件时就把过期时间写进它自己的 `assets.expires_at`**（改设置只影响以后生成的，缩短不会追溯删用户已有作品、延长也复活不了已删的）→ dispatcher 里新增一个每小时跑一次的 `sweep_expired_assets` 真的删 R2 对象、把 `mirror_status` 标成 `expired`、把 job 的 `output` 改成 `{asset_key: null, asset_expired: true}`。数据库是唯一真相，R2 生命周期规则最多只当一个更宽的兜底（比如 180 天）。`GET /jobs` 只返回保留期内的作业，画廊本来就要求 `mirror_status='done'` 所以过期的自动消失。`jobs.output` 现在只存 `asset_key`，对外的 `asset_url` 在响应层（`schemas.py`）用 `R2_PUBLIC_BASE_URL` 现拼——以后换域名不会让已存的行失效；迁移 `0014` 把旧的 `/jobs/{id}/asset` 路径改写成 key（测试库 5 行，验证过前后数量对得上）。

**前端整体变简单了**：`api.assetBlobUrl()` 和两个页面里一整套 `IntersectionObserver` + blob URL 的懒加载/回收逻辑全删了，改成直接 `<img loading="lazy" src>`、`<video preload="metadata">`、`<audio preload="none">`（浏览器自己负责懒加载、缓存、Range）。**下载按钮是个真实的坑**：跨域的 `<a download>` 会被浏览器忽略（直接导航过去），所以改成 `fetch(url)` 拿到 blob 再保存——这要求桶上有 CORS 规则；如果 fetch 失败就退回 `window.open` 新标签页。测试时第一次点下载真的走了这个退路，一查 R2 返回的 CORS 头是对的（`curl -H Origin` 验证过），而手动 `fetch` 又成功了——推测是 `<img>` 先加载过同一个 URL（不带 `Origin`），缓存里那份没有 CORS 头，后面的 `fetch` 复用了它；改成 `cache: "reload"` 后再点，走的是 blob 保存、文件名正确、没退回新标签页。这个原因是推断出来的，没有把"修复前失败"完整复现，如实记一笔。隐私页（`content/marketing/en.ts`）把"everything you create is private, visible only to you"改成如实描述：私有=不在公开画廊里，文件放在长且几乎不可能猜到的地址上，但拿到直链的人都能打开；同时写明目前保留 90 天。

**真实验证（测试库 + `voicica-v2-dev` 桶，不是 mock）**：真提交了一个 TTS 任务 → `202` → dispatcher 处理 → `output` 里只有 `asset_url`，指向 `pub-…r2.dev/jobs/{uuid}.mp3` → 不带任何鉴权头直接 GET 是 200、`audio/mpeg`、`Cache-Control: public, max-age=604800` → `Range: bytes=0-99` 返回 206 → 旧的 `/jobs/{id}/asset` 路由 404、OpenAPI 里已经没有这个路由 → 匿名访问 `/gallery` 能看到它。过期这一半：确认写进去的 `expires_at` 正好是 90 天后；手动把它改成一小时前，跑真实的 `sweep_expired_assets` → 删了 1 个 → 文件再 GET 是 404 → `GET /jobs` 返回 `{asset_expired: true, asset_url: null}` → 从画廊消失 → 再跑一次删 0 个（幂等）。浏览器里：历史页图片直接从 `r2.dev` 加载（网络请求里看到 200、1024px、`loading=lazy`、页面里零个 `blob:` 元素）、过期的那条显示"· Expired"、查看弹窗+下载按钮、Explore 的 Images/Voices 两个 tab 都对。**验证边界**：这个自动化浏览器标签页 `visibilityState` 一直是 `hidden`，所以（a）`loading="lazy"` 的图片不会加载，把同一个元素改成 `eager` 就立刻加载成功（1024×1024），确认不是代码/URL 的问题，（b）音频/视频没法在这个环境里真的解码播放（`readyState` 卡在 0，之前 CLAUDE.md 里已经记过一次同样的限制）——音频只验证了服务端返回（200/206、`audio/mpeg`）和元素的 `src` 是直链，真实播放需要用户自己在浏览器里点一下确认。测试用的 e2e 账号没有图片作业，所以往测试库里造了两条指向真实文件的临时作业行，验证完已经删掉，临时的 `/dev-signin` 页面也删了，浏览器里的 Firebase 登录态清掉了，验证用的本地服务也都关了。

**还没做、需要用户操作的（生产上线顺序很重要）**：①Cloudflare 后台把 `assets.voicica.ai` 绑到生产桶 `voicica-v2`（Settings → Custom Domains）；②给生产桶加 CORS 规则（`GET`/`HEAD`，来源 `https://voicica.ai` 和 `https://www.voicica.ai`）；③Render 两个服务的 `R2_PUBLIC_BASE_URL` 改成 `https://assets.voicica.ai`（`.env.production` 里已留 TODO）；④提交并 push（Render 自动部署后端，构建时会跑迁移 `0014`），紧接着本地跑一次 `npm run deploy` 发前端——两边先后部署的间隙里，旧前端拿到新格式的 `asset_url` 会短暂看不了媒体，上线前的个人项目可以接受，但两步要连着做。②③④之间不用等域名生效：`r2.dev` 那个地址在绑域名之前也能用，只是没有 CDN 缓存。可选的：给生产桶加一条宽的生命周期兜底规则（180 天，`jobs/` 前缀）清理孤儿文件；生产桶里早先混进去的测试文件留着无害。
