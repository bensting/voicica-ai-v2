...
- 真实浏览器验证：泰语 Google 声音、西语墨西哥 Azure 声音都真实生成播放成功；语言下拉改按基础语言分组后，Popular 14 + All languages 69 = 83 对上；选中"Spanish"能看到 Azure 和 Google 的声音混在一起、按 provider 筛选也对；中文方言各自显示正确名字不再撞车。
