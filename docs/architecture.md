...
- **Model pinned to `s2.1-pro`** via the `model` header — verified against Fish Audio's current docs (2026-09-12): it's their recommended production model and also the server-side default if the header is omitted, but pinned explicitly rather than relying on an undocumented default. No "v2" model exists (naming is generational: `s1`/`s2-pro`/`s2.1-pro`/`s2.1-pro-free`/`drama-3-preview`); `speech-1.5`/`speech-1.6` (used by the prior project's adapter) were deprecated 2026-02-28.
