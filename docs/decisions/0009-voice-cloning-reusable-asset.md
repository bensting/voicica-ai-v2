...
**Negative / open items:**
- ~~Exact credit cost of *training* a voice~~ **Resolved: free.** Implemented and verified — training is `estimated_cost=0`, going through the ordinary hold->settle lifecycle at that amount rather than skipping it (for audit-trail parity). This ports the prior project's own real, already-shipped pricing (it never charged for cloning, only for *using* a clone to generate speech, at the ordinary per-character TTS rate) rather than inventing a number.
- Whether Azure/Google support an equivalent durable custom-voice concept (vs. only inline, per-call zero-shot cloning) is unconfirmed — checked when those adapters are actually built. **Still open**: only Fish Audio is implemented (`app/providers/fish_audio.py`'s `_train`/`delete_voice_model`, `services/jobs.py submit_voice_model_training`, `POST/GET/DELETE /voice-models`) — verified end-to-end against the real API and a real browser round trip (train -> list -> speak -> delete).
- **New, found via that real test**: `jobs.voice_model_id`'s FK needs `ON DELETE SET NULL`, not Postgres's default `RESTRICT` — deleting a voice model a job had ever referenced (its own training job, or a TTS job that used it) otherwise fails with a `ForeignKeyViolationError`. Fixed in migration `0007` — a job's history should survive its voice being deleted, not pin it forever.
