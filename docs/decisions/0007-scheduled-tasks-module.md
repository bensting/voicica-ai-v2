...
**Negative / open items:**
- ~~Scheduling mechanism is explicitly undecided~~ **Resolved by [ADR 0014](0014-background-job-execution.md)**: arq's cron feature (adopted there for a different, independently-arrived-at reason — decoupling provider calls from the request path) turned out to be this module's scheduling mechanism too. The stuck-job sweep described below becomes an arq cron job; `sync_catalog.py` can move to one once a sync cadence is picked, but still runs manually (`python -m app.scheduled.sync_catalog`) for now — nothing forces the move immediately.
