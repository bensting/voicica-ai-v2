...
**Negative / open items:**
- Scheduling mechanism is explicitly undecided — revisit once backend hosting is chosen.
- Per-provider sync frequency and exact source endpoints are undecided — decided per-adapter, when built.
- Sweep timeout threshold (the 10–15 min figure is Kie's own polling-guidance number, not yet confirmed as our sweep policy) is open.
