...
erDiagram
    ...
    jobs {
        ...
        int tries "ADR 0026 — replaces arq's own retry counter, default 0"
        timestamp run_after "nullable — ADR 0026, delayed-retry scheduling; NULL = claimable now"
        timestamp created_at
        timestamp updated_at
    ...
    assets {
        ...
        string job_id FK
        string r2_key
        string mirror_status "pending | done | failed | expired"
        timestamp mirrored_at "nullable"
        timestamp expires_at "nullable — stamped at creation: mirrored_at + app_settings.asset_retention_days (ADR 0027)"
    }
    voice_catalog {
        string id PK
...
- **`assets`**: one per successfully mirrored job ([ADR 0004](decisions/0004-asset-mirroring-r2-retention.md)). Absence of a row (or `mirror_status != done`) means there's nothing in R2 for that job. Since [ADR 0027](decisions/0027-public-asset-delivery-and-retention.md), `jobs.output` holds only the object key (`asset_key`; the public `asset_url` is derived at response time), and an hourly sweep deletes the R2 object once `expires_at` passes, sets `mirror_status = expired`, and rewrites the job's `output` to `{asset_key: null, asset_expired: true}`.
