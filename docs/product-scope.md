...
**Top-up mechanism — resolved, [ADR 0024](decisions/0024-credit-purchases-stripe-checkout.md): Stripe Checkout, three one-time credit packs** ($9/1,000, $99/12,500, $199/29,500 credits — real prices, not placeholders). Built and verified end to end against real Stripe test-mode infrastructure — a real Checkout page, a real test-card payment, a real signature-verified webhook crediting the wallet exactly once (idempotency confirmed against a simulated redelivery too). Caught and fixed a real bug along the way (a `stripe` SDK object doesn't support `.get(...)`, only subscript access — see ADR 0024) and surfaced a real, still-open gap: no automated reconciliation yet for a purchase that's genuinely paid but stuck `pending` (e.g. a webhook delivery failure). The frontend "buy credits" screen doesn't exist yet. The signup bonus (`app_settings.signup_bonus_credits`, currently 50) remains the only way a new user gets credits without paying.
...
- **`/get`** (own minimal layout, deliberately outside `(marketing)`): the one URL meant for every future paid/campaign traffic source ([ADR 0021](decisions/0021-dedicated-landing-page-and-analytics.md)) — no nav, no exits besides "download the Android app" or "get started on the web," CTA order decided server-side from the request's own `User-Agent`. `noindex`, not in the sitemap (organic search shouldn't surface a page whose pitch duplicates the homepage's). GA4 (via Firebase Analytics) records page views and which CTA was clicked, scoped to marketing surfaces only.
