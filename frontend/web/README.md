...
**Voice picker**: built and verified — `components/VoiceSheet.tsx`, opened from a "Select a voice" row on the TTS create page. No target-market restriction on which languages are selectable (all 158 locales Azure/Google's real catalogs cover) — instead the payload stays small by fetching **one language's voices at a time**: `api.getLocales()` loads the (cheap) language list once, `api.getVoices(undefined, locale)` re-fetches only when the selected language changes, mirroring the prior project's `useVoices()` hook. The language dropdown groups a small "Popular" shortlist (one representative locale per common language, target market first — `POPULAR_LOCALES` in `VoiceSheet.tsx`) above "All languages" (everything else), also matching the prior project's pattern. Search/gender/provider filter client-side within the currently-loaded language (provider filter only renders once more than one provider is present). `lib/locale-names.ts` formats locale codes ("es-MX" → "Spanish (Mexico)") via the browser's own `Intl.DisplayNames` — no lookup table needed, unlike the prior project's hand-maintained one — and cleans up Google's raw voice-id display names ("th-TH-Chirp3-HD-Achernar" → "Chirp3 HD Achernar"; Azure's own names need no cleanup). No play-preview button — deliberately cut (previewing would mean either paying to regenerate on every tap or building a sample-caching pipeline; punted rather than guessing which).
