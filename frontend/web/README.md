...
**Voice picker**: built and verified — `components/VoiceSheet.tsx`, opened from a "Select a voice" row on the TTS create page. Fetches `api.getVoices()` once (the backend already scopes this to the ~270-voice target market, not the full multi-language catalog — `backend/README.md`), then filters client-side by search text, locale, gender, and provider (provider filter only renders once more than one provider is present in the data). `lib/locale-names.ts` formats locale codes ("es-MX" → "Spanish (Mexico)") via the browser's own `Intl.DisplayNames` — no lookup table — and cleans up Google's raw voice-id display names ("th-TH-Chirp3-HD-Achernar" → "Chirp3 HD Achernar"; Azure's own names need no cleanup). No play-preview button — deliberately cut (previewing would mean either paying to regenerate on every tap or building a sample-caching pipeline; punted rather than guessing which).
